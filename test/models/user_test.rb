...
class UserTest < ActiveSupport::TestCase
  test 'valid user' do
    user = User.new(username: 'utente1', is_admin: false, password: 'test_password_utente1')
    assert user.valid?
  end
  ...
  test 'invalid if username already taken' do
    user = User.new(username: 'utente1', is_admin: false, password: 'test_password_utente1')
    user_dup = User.new(username: 'utente1', is_admin: false, password: 'test_password_utente1')
